...
export function TechCard({ item, accent }: TechCardProps) {
  return (
    <div className="group h-full">
      <div
        className={`flex h-full flex-col items-center gap-3 rounded-2xl border border-border bg-white px-2.5 pt-5.5 pb-4.5 transition-[transform,border-color,box-shadow] duration-[260ms] group-hover:-translate-y-1.5 group-hover:shadow-[0_16px_32px_-20px_rgba(11,15,26,0.4)] ${HOVER_BORDER_CLASS[accent]}`}
      >
        {item.logo ? (
